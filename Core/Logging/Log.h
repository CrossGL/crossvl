...
class Log {
public:
  explicit Log(std::string_view name) : m_name(name) {}
...
private:
  ...
  void DispatchLog(LogLevel level, std::string_view message, Args &&...args) {
    std::string msg =
        std::vformat(std::string(message),
                     std::make_format_args(std::forward<Args>(args)...));
    Write(level,
          LogMessage{.Payload = std::string_view(msg.data(), msg.size())});
  }
...
#define CGL_LOG(Name, Level, ...)                                              \
  LOG_CONCATENATE(g_Log, Name).Level(__VA_ARGS__)
#else
