...
void ShaderCompiler::ReportResult(const ShaderCompileResult &result,
  ...
  if (result.Status == ShaderCompileStatus::Success) {
    ...
    CGL_LOG(ShaderCompiler, Error, "Failed to compile shader: {}  Error: {}",
            source, result.Message);
  } else if (result.Status == ShaderCompileStatus::HasWarnings) {
    CGL_LOG(ShaderCompiler, Warn,
            "Shader compiled with warnings: {}  Warning: {}", source,
            result.Message);
  } else {
    CGL_LOG(ShaderCompiler, Error, "Failed to compile shader: {}  Error: {}",
            source, result.Message);
...
ShaderCompileResult ShaderCompiler::Compile(const ShaderSource &shader,
  ...
  HRESULT hr = D3DCompile(shader.SourceData.c_str(), shader.SourceData.length(),
                          shader.Name.c_str(), config.Defines.data(),
                          D3D_COMPILE_STANDARD_FILE_INCLUDE,
                          config.EntryPoint.data(), config.Target.data(),
