...
D3D11RendererImpl::D3D11RendererImpl(HWND window) : m_window(window) { Init(); }

D3D11RendererImpl::~D3D11RendererImpl() { Shutdown(); }

void Graphics::D3D11RendererImpl::CreateSizeDependentResources(u32 width,
                                                               u32 height) {
  ...
  DXCall(hr = m_device->CreateRenderTargetView(m_backBufferTexture.Get(),
                                               nullptr, &m_backBuffer));
}
...
void D3D11RendererImpl::CreateDevice() {
  ...
  if (!m_device) {
    ...
    DXCall(hr = ::D3D11CreateDevice(nullptr, D3D_DRIVER_TYPE_HARDWARE, nullptr,
...
#ifdef CGL_BUILD_DEBUG
    {
      ...
      if (SUCCEEDED(m_device.As(&debug))) {
        ...
        if (SUCCEEDED(debug.As(&infoQueue))) {
          infoQueue->SetBreakOnSeverity(D3D11_MESSAGE_SEVERITY_CORRUPTION,
                                        true);
...
void D3D11RendererImpl::CreateSwapChain(u32 width, u32 height) {
  ...
  DXCall(hr = dxgiOutput->GetDisplayModeList(DXGI_FORMAT_R8G8B8A8_UNORM, 0,
                                             &numModes, nullptr));
  ...
  if (numModes > 0) {
    ...
    for (u32 i = 0; i < numModes; ++i) {
      ...
      f32 rate = (f32)modes[i].RefreshRate.Numerator /
    ...
    if (bestMatchIndex != -1) {
      refreshRateRational.Numerator =
          modes[bestMatchIndex].RefreshRate.Denominator;
      refreshRateRational.Denominator =
          modes[bestMatchIndex].RefreshRate.Numerator;
    }
  ...
  DXCall(hr = dxgiFactory->CreateSwapChain(m_device.Get(), &swapChainDesc,
                                           &m_swapChain));
  assert(m_swapChain);
...
void D3D11RendererImpl::ResizeSwapchain(u32 width, u32 height) {
  ...
  HRESULT hr =
      m_swapChain->ResizeBuffers(1, width, height, DXGI_FORMAT_UNKNOWN,
                                 DXGI_SWAP_CHAIN_FLAG_ALLOW_MODE_SWITCH);
  if (hr == DXGI_ERROR_DEVICE_REMOVED || hr == DXGI_ERROR_DEVICE_RESET) {
    // Shutdown and restart the DX11 renderer
    std::string err = (hr == DXGI_ERROR_DEVICE_REMOVED)
                          ? "The device was removed"
                          : "Error during device reset";
    CGL_LOG(D3D11RendererImpl, Error,
            "Swapchain resize error: {0}. Restarting...", err);

    Shutdown();
    Init();
  }

  CGL_LOG(D3D11RendererImpl, Trace, "D3D11 SwapChain Resized to {0}x{1}", width,
          height);
}
} // namespace CGL::Graphics
