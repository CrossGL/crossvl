...
inline void
SetDebugObjectName([[maybe_unused]] _In_ T *resource,
...
void Renderer::SetPrimitiveTopology_D3D11(PrimitiveType topology) {
  ...
  impl->GetContext()->IASetPrimitiveTopology(
      Mapping::PrimitiveTopology[size_t(topology)]);
}
...
void Renderer::SetVertexBuffer_D3D11(const VertexBuffer &buffer) {
  assert(GetImpl() && GetImpl()->GetContext());
  GetImpl()->GetContext()->IASetVertexBuffers(
      0, 1, buffer.Buffer.GetAddressOf(), &buffer.Stride, &buffer.Offset);
}

void Renderer::SetIndexBuffer_D3D11(const IndexBuffer &buffer) {
  assert(GetImpl() && GetImpl()->GetContext());
  GetImpl()->GetContext()->IASetIndexBuffer(buffer.Buffer.Get(), buffer.Format,
                                            0);
}

ShaderCompileResult
Renderer::CompileVertexShader_D3D11(const ShaderSource &source,
  ...
  ShaderCompileResult result =
      ShaderCompiler::Compile(source, cfg, outShader->Blob);
  ...
  if (result.Status != ShaderCompileStatus::Failure) {
    ...
    DXCall(hr = GetImpl()->GetDevice()->CreateVertexShader(
               ...
               outShader->Blob->GetBufferSize(), nullptr, &outShader->Shader));
    ...
    DXCall(hr = D3DReflect(outShader->Blob->GetBufferPointer(),
    ...
    DXCall(hr = GetImpl()->GetDevice()->CreateInputLayout(
               inputLayout.data(), (u32)inputLayout.size(),
               outShader->Blob->GetBufferPointer(),
...
ShaderCompileResult
Renderer::CompilePixelShader_D3D11(const ShaderSource &source,
...
IndexBuffer Renderer::CreateIndexBuffer_D3D11(const BufferSource &source) {
  ...
  ib.Format = (source.TypeSize == sizeof(u16) ? DXGI_FORMAT_R16_UINT
...
void Renderer::CreateContantBuffer_D3D11(const BufferSource &source,
...
void Renderer::SetConstantBufferData_D3D11(ID3D11Buffer *buffer,
                                           const void *data, size_t size) {
  assert(buffer && GetImpl() && GetImpl()->GetContext());

  D3D11_MAPPED_SUBRESOURCE mappedResource;
  if (SUCCEEDED(GetImpl()->GetContext()->Map(buffer, 0, D3D11_MAP_WRITE_DISCARD,
                                             0, &mappedResource))) {
    // Ensure the data size fits within the mapped resource
    assert(size <=
           mappedResource.RowPitch); // RowPitch is used for 1D and 2D data
...
void Renderer::SetContantBuffer_D3D11(ShaderType type, u32 startSlot,
                                      const ComPtr<ID3D11Buffer> &buffer) {
  assert(GetImpl() && GetImpl()->GetContext() && buffer);

  switch (type) {
  case CGL::Graphics::ShaderType::Vertex:
    GetImpl()->GetContext()->VSSetConstantBuffers(startSlot, 1,
                                                  buffer.GetAddressOf());
    break;
  case CGL::Graphics::ShaderType::Hull:
    GetImpl()->GetContext()->HSSetConstantBuffers(startSlot, 1,
                                                  buffer.GetAddressOf());
    break;
  case CGL::Graphics::ShaderType::Domain:
    GetImpl()->GetContext()->DSSetConstantBuffers(startSlot, 1,
                                                  buffer.GetAddressOf());
    break;
  case CGL::Graphics::ShaderType::Geometry:
    GetImpl()->GetContext()->GSSetConstantBuffers(startSlot, 1,
                                                  buffer.GetAddressOf());
    break;
  case CGL::Graphics::ShaderType::Pixel:
    GetImpl()->GetContext()->PSSetConstantBuffers(startSlot, 1,
                                                  buffer.GetAddressOf());
    break;
  case CGL::Graphics::ShaderType::Compute:
    GetImpl()->GetContext()->CSSetConstantBuffers(startSlot, 1,
                                                  buffer.GetAddressOf());
    break;
...
void Renderer::DrawIndexed_D3D11(u32 indexCount, u32 startIndex,
                                 u32 baseVertex) {
  assert(GetImpl() && GetImpl()->GetContext());
