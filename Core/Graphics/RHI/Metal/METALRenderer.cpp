...
void Renderer::BeginFrame_METAL() {
  ...
  rpDescriptor->colorAttachments()->object(0)->setTexture(
      GetImpl()->GetDrawable()->texture());
  rpDescriptor->colorAttachments()->object(0)->setLoadAction(
      MTL::LoadActionClear);
  rpDescriptor->colorAttachments()->object(0)->setClearColor(
      MTL::ClearColor{r, g, b, a});
  rpDescriptor->colorAttachments()->object(0)->setStoreAction(
      MTL::StoreActionStore);

  auto rCmdEncoder =
      GetImpl()->GetCmdBuffer()->renderCommandEncoder(rpDescriptor);
