...
namespace CGL::Graphics
{
    ...
    public:
        ...
        template <typename T>
        void CreateContantBuffer(const BufferSource& source, ConstantBuffer<T>& outBuffer);
        template <typename T>
        void SetConstantBufferData(const ConstantBuffer<T>& buffer, const T& data);
        template <typename T>
        void SetContantBuffer(ShaderType shaderType, u32 startSlot, const ConstantBuffer<T>& buffer);
