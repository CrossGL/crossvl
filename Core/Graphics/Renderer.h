...
public:
  ...
  template <typename T>
  void CreateContantBuffer(const BufferSource &source,
                           ConstantBuffer<T> &outBuffer);
  template <typename T>
  void SetConstantBufferData(const ConstantBuffer<T> &buffer, const T &data);
  template <typename T>
  void SetContantBuffer(ShaderType shaderType, u32 startSlot,
                        const ConstantBuffer<T> &buffer);
...
private:
  ...
  ShaderCompileResult CompileVertexShader_D3D11(const ShaderSource &source,
                                                VertexShader *outShader);
  ShaderCompileResult CompilePixelShader_D3D11(const ShaderSource &source,
                                               PixelShader *outShader);
  void CreateContantBuffer_D3D11(const BufferSource &source,
                                 ComPtr<ID3D11Buffer> &outBuffer);
  void SetConstantBufferData_D3D11(ID3D11Buffer *buffer, const void *data,
                                   size_t size);
  void SetContantBuffer_D3D11(ShaderType type, u32 startSlot,
  ...
  void DrawIndexed_D3D11(u32 indexCount, u32 startIndex = 0,
                         u32 baseVertex = 0);
