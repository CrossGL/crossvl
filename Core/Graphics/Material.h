...
template <typename T> struct ShaderProgram {
  ShaderState State;
  ShaderSource Source;
  T Shader;
};
...
public:
  ...
  inline const ShaderProgram<VertexShader> *GetVertexShader() const noexcept {
    return m_vs.get();
  }
  inline const ShaderProgram<PixelShader> *GetPixelShader() const noexcept {
    return m_ps.get();
  }
