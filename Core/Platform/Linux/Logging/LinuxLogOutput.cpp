...
ConsoleLogOutput::~ConsoleLogOutput() { endwin(); }

void ConsoleLogOutput::Write([[maybe_unused]] LogLevel level,
...
FileLogOutput::FileLogOutput(std::string_view name) : m_name{name} {
...
FileLogOutput::~FileLogOutput() { file.close(); }

void FileLogOutput::Write([[maybe_unused]] LogLevel level,
                          const std::string &message) {
  if (file.is_open()) {
    file << message;
    file.flush();
...
CallbackLogOutput::CallbackLogOutput(
