...
ConsoleLogOutput::~ConsoleLogOutput() { ::FreeConsole(); }
...
void ConsoleLogOutput::Write(LogLevel level, const std::string &message) {
  ...
  std::ignore =
      ::WriteConsoleA(m_console, message.data(),
                      static_cast<DWORD>(message.size()), nullptr, nullptr);
  // Restore color
...
FileLogOutput::FileLogOutput(std::string_view name)
  ...
  m_file = ::CreateFileA(m_name.c_str(), GENERIC_WRITE, 0, nullptr,
...
void FileLogOutput::Write(LogLevel level, const std::string &message) {
  ...
  UNREFERENCED_PARAMETER(level);
  std::ignore =
      ::WriteFile(m_file, message.data(), static_cast<DWORD>(message.size()),
                  nullptr, nullptr);
}
