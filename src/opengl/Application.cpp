...
bool Application::initSDL()
{
    ...
    else
    {
        ...
        window = SDL_CreateWindow("Visualizer-SDL2OpenGL", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_OPENGL | SDL_WINDOW_SHOWN);
...
void Application::handleImGui()
{
    if (ImGui::Begin("Shader Loader")) {
        ...
        if (ImGui::Button("Load Fragment Shader")) {
            ImGuiFileDialog::Instance()->OpenDialog("ChooseFragmentShader", "Choose Fragment Shader", ".frag,.glsl,.txt");
        }
        ...
        if (ImGui::Button("Load Shaders")) {
            if (strlen(vertexPathAdd) > 0 && strlen(fragmentPathAdd) > 0) {
                ...
                shader = new Shader(std::filesystem::relative(vertexPathAdd).string(), std::filesystem::relative(fragmentPathAdd).string());
