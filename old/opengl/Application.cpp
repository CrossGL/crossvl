...
Application::Application()
    : window(nullptr), gContext(nullptr), shader(nullptr), startTime(0.0f),
      endTime(0.0f), model(glm::mat4(1.0f)) // Identity matrix
{
  cube = new Cube();
}
...
bool Application::initSDL() {
  ...
  if (SDL_Init(SDL_INIT_VIDEO) < 0) {
    ...
    window =
        SDL_CreateWindow("Visualizer-SDL2OpenGL", SDL_WINDOWPOS_UNDEFINED,
  ...
  if (ImGui::Begin("Shader Loader")) {
    ...
    if (ImGui::Button("Load Vertex Shader")) {
      ImGuiFileDialog::Instance()->OpenDialog(
          "ChooseVertexShader", "Choose Vertex Shader", ".vert,.glsl,.txt");
    }

    if (ImGui::Button("Load Fragment Shader")) {
      ImGuiFileDialog::Instance()->OpenDialog(
          "ChooseFragmentShader", "Choose Fragment Shader", ".frag,.glsl,.txt");
    }
    ...
    if (ImGui::Button("Load Shaders")) {
      if (strlen(vertexPathAdd) > 0 && strlen(fragmentPathAdd) > 0) {
        ...
        shader =
            new Shader(std::filesystem::relative(vertexPathAdd).string(),
        ...
        std::cerr << "Please select both vertex and fragment shader files."
                  << std::endl;
      }
    }
