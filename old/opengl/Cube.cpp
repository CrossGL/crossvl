...
Cube::Cube()
    : vboID(0), iboID(0), position(0.0f), scale(1.0f), rotationAngle(0.0f),
      axis(1.0f), model(glm::mat4(1.0f))
...
void Cube::update(float deltaTime) {
  ...
  model = glm::translate(glm::mat4(0.1f), position) *
          glm::scale(glm::mat4(0.1f), scale) *
          glm::rotate(glm::mat4(0.1f), rotationAngle, axis);
}
...
void Cube::render(GLuint gProgramID) {
  ...
  glVertexAttribPointer(gVertexPos3DLocation, 3, GL_FLOAT, GL_FALSE,
  ...
  glUniformMatrix4fv(glGetUniformLocation(gProgramID, "model"), 1, GL_FALSE,
                     glm::value_ptr(model));
  glDrawElements(GL_TRIANGLES, static_cast<GLsizei>(indexData.size()),
                 GL_UNSIGNED_INT, 0);
...
void Cube::setScale(const glm::vec3 scale) { this->scale = scale; }

void Cube::setRotationAngle(float rotationAngle) {
  this->rotationAngle = rotationAngle;
}

void Cube::setAxis(const glm::vec3 axis) { this->axis = axis; }
